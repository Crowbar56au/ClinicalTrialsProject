...
/**
 * UPDATE 16 feb 2013  update code so it support multivalues in view columns (dbcolumn) and fields (dblookup)
 * UPDATE 14 feb 2013  added various tweaks thanks to Karsten Lehmann and Ulrich Krause
 *
 *
 ...
 *
 ***********************************
 *  Dblookup - java version
 *             including cache,sort and unique, key, fieldname/colnr functionality
 *********************************
 *
  * from (Java)/SSJS you can call this function e.g.
 *    (get all unique, sorted states from all documents)
 *    return org.openntf.Utils.Dblookup(database,"AllNames",new org.openntf.Utils.Params(true,true,true,"", "State"));
 *  or
 *    (get all values from second column where key=Abby)
 *    return org.openntf.Utils.Dblookup(database,"AllNames",new org.openntf.Utils.Params(true,true,true,"Abby", "1"));
 *
 *********************************
 *  Dbcolumn - java version
 *             including categoryfilter,cache,sort and unique functionality
 *********************************
 *
 *
 * from (Java)/SSJS you can call this function e.g.
 ...
 *  or
 *    return org.openntf.Utils.Dbcolumn(database,"AllNames",1);  // without cache,sort,categoryfilter and unique
 *
...
public class Utils implements Serializable {
  ...
  /**
   * Dbcolumn - Java version of @Dbcolumn
   *   sort = false
   *   unique = false
   *   cache = false
   * @param db
   ...
   *  uses Params object
     ...
     * @throws NotesException
     */
  public static ArrayList<String>Dbcolumn(Database db, String viewname, int columnNr,Params params ) throws NotesException{
   ...
   * @throws NotesException
   */
  public static ArrayList<String>Dbcolumn(String server,String dbpath, String viewname, int columnNr) throws NotesException{
   ...
   * @throws NotesException
   */
  public static ArrayList<String>Dbcolumn(String server,String dbpath, String viewname, int columnNr, Params params) throws NotesException{
    Database db = getSession().getDatabase(server, dbpath);
    return DbcolumnCore(db,viewname,columnNr, params);
  }
   ...
   * @throws NotesException
   */
  private static ArrayList<String> DbcolumnCore(Database db, String viewname, int columnNr,Params params) throws NotesException{
    // generate cachekey
    String cachekey = Utils.replaceAll("dbc_"+db.getFilePath()+"_"+viewname+"_"+String.valueOf(columnNr), " ", "_");
...
//    System.out.println(params.getKey());  
  ...
  * This method checks if the input string is a valid Australian phone number.
  ...
  */  
  public static boolean isPhoneNumberValid(String phoneNumber){  
      ...
      String expression = "^\\({0,1}((0|\\+61)(2|4|3|7|8)){0,1}\\){0,1}(\\ |-){0,1}[0-9]{2}(\\ |-){0,1}[0-9]{2}(\\ |-){0,1}[0-9]{1}(\\ |-){0,1}[0-9]{3}$";
