...
public class LookupAllSponsorData_1 implements Serializable {
		...
		try {
			...
			if (c != null && c.equalsIgnoreCase("")) {
				...
				sysdbpath = ebsSystemProfile
						.getItemValueString("ClientWebPath");
				...
				if (cwb != null) {
					...
					if (view != null) {
						...
						if (tmpdoc != null) {
							clienttype = tmpdoc
									.getItemValueString("ClientType");
							if (clienttype.toLowerCase().contains("agent")) {
								isagent = true;
							} else if (clienttype.toLowerCase().contains(
									"sponsor")) {
								issponsor = true;
							}
							...
							entryMap.put("name", tmpdoc
									.getItemValueString("ClientName"));
							if (view != null) {
								...
								while (entry != null) {
									...
									sponsor.put("id", doc
											.getItemValueString("ClientID"));
									sponsor.put("name", doc
											.getItemValueString("ClientName"));
								...
								entryMap.put("count", vec.getCount());
					...
					// Return a JSON string generated from our JsonJavaObject
					if (varname != null) {
						strValue = JsonGenerator.toJson(
								JsonJavaFactory.instanceEx, returnJSON);
						strValue = "var " + varname + "=" + strValue;
					} else {
						strValue = JsonGenerator.toJson(
								JsonJavaFactory.instanceEx, returnJSON);
